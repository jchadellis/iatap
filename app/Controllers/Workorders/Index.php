...
class Index extends BaseController
{
    ...
    protected $depts = [
        ...
        ['description' => 'fabrication', 'id' => ['S135', 'RM135', 'S130', 'RM130','S132', 'RM132', 'S135', 'RM135', 'S200', 'RM200', 'S205', 'RM205','S190', 'RM190']],
    ...
    public function index($id = 0)
    {
        ...
        $results = $service->getOpenWorkOrders(($this->depts[$id]['id']) ?? []);

        $breadcrumbs = [
            ['name' => 'Dashboard', 'is_active' => false, 'url' => '/dashboard'],
            ['name' => 'Production', 'is_active' => false, 'url' => '/production'],
            ['name' => 'Work Orders - '. strtoupper($this->depts[$id]['description']), 'is_active' => true, 'url' => '#']
        ];
        ...
        $this->data = ['site_name' => 'iATAP', 'breadcrumbs' => $breadcrumbs, 'title' => 'Production', 'content' => view('workorders/index', ['data' => $results, 'depts' => $this->depts, 'id' => $id]) ];
        $this->data['js'] = view('workorders/index.js.php');

        return view('template/index', $this->data);  
    }

    public function workorder( $id, $dept )
    {
       
        $results = $this->service->getWorkorderDetails($id, $this->depts[$dept]['id']);
    ...
    public function print( )
    {
        ...
        foreach ($operations as &$op) {
            $op->requirements = $db->query("SELECT * FROM requirements_cache WHERE base_id = '$op->base_id' AND sub_id = '$op->sub_id' AND seq_no = '$op->sequence_no' ORDER BY CAST(piece_no AS INTEGER ) ASC")->getResult();
            // $remoteModel = new SqlbaseModel(); 
            // $op->requirements = $remoteModel->getData('http://vatap/mvc/public/api/getrequirements/'.$post->base_id.'/'.$op->sub_id.'/'.$op->sequence_no ); 
            $count = 0;
            foreach ($op->requirements as $row) {
                ...
                $row->issued = ($row->issued_qty >= $row->calc_qty ) ? true : false; 
                $count++;
            }
        }
        ...
        $operations[0]->{'deliver_to'} = $post->deliver_to; 
        $data = $operations[0];

        $searchId = $data->resource_id;
        $dept = null;
        
        foreach ($this->depts as $dept) {
            if (in_array($searchId, $dept['id'])) {
                $dept = strtoupper($dept['description']);
                break;
            }
        }


        $pdf->print($outputFile, $operations[0], $dept);
    
        $fullpath = WRITEPATH . 'uploads/' . $outputFile;
    
        $email = \Config\Services::email();
        $email->setTo('warehouse@atap.com');
        //$email->setTo('jeremy.ellis@atap.com');
        $email->setCC('jeremy.ellis@atap.com'); 
        $email->setSubject('Parts List Request PDF');
         
        $body = "Attached is the parts list request form for Work Order: $data->base_id / SUB ID: $data->sub_id / SEQUENCE NO: $data->sequence_no / RESOURCE ID: $data->resource_id / DEPT: $dept"; 
    ...
    public function open_workorders()
    {
        ini_set('memory_limit', '256M');
        // All requirements, grouped by work order
        $results = $this->service->getGroupedRequirementsByWorkOrder();

        return view('workorders/open_workorders', ['data' => $results]);
    }
