...
class MainController extends BaseCommand
{
    protected $group       = 'Generators';
    protected $name        = 'make:main-controller';
    protected $description = 'Generates a new main iATAP controller.';
    ...
    private function createTemplates($path, $template, $className):bool
    {
        ...
        foreach($files as $key => $file)
        {
            ...
            if($key === 'controller' ){
                ...
                $templatePath = APPPATH . 'Commands/Generators/Views/index.tpl.php';
                $template = file_get_contents($templatePath);
                if(!write_file( $target, $template )){
                    CLI::error('Failed to create directory: ' . $file);
                    return false; 
                ...
                $templatePath = APPPATH . 'Commands/Generators/Views/index.tpl.js.php';
                $template = file_get_contents($templatePath);
                if(!write_file( $target, $template )){
                    CLI::error('Failed to create directory: ' . $file);
                    return false; 
