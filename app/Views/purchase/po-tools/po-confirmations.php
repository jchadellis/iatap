...
<table class="table table-striped table-bordered">
    ...
    <tbody>
        ...
        <tr data-vendor="<?= $row->vendor_id ?>" data-purchase_order="<?= $row->id ?>" data-promise_date="<?= $row->true_promise->format('m-d-Y') ?>">
            <td class="text-center col-1 <?// $row->color ?>"><?= $row->id ?> <?= $row->confirmed == 'X' ? '<span class="badge bg-success">C</span>' : '' ?></td>
            <td class="text-center col-2" data-order="<?= $row->true_promise->format('Y-m-d') ?>" ><?= $row->true_promise->format('m-d-Y') ?></td>
...
<div class="modal" id="content_modal" tabindex="-1">
